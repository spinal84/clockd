...
module Nokogiri::XML::Searchable
    @@struct = Dir[File.dirname(__FILE__) + '/legacy-libtime/struct*.html'].grep_v(/-members\.html$/).
...
class CommentsHandler
	...
	prj_dir = File.expand_path(@html_file).sub(/(\/clockd\/).*$/, "$1")
...
path = File.expand_path(ARGV[0]) rescue File.dirname(__FILE__) + '/legacy-libtime'
